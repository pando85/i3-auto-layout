...
Release a new version of i3-auto-layout using the release script and CI pipeline.
...
- New CLI options or configuration options
- New window manager support
...
The changelog is generated from conventional commits and grouped by type (Added, Fixed, Documentation, Build, Refactor, Styling, Testing). Release commits are excluded.
...
- `rust.yml`: Builds for Linux, runs tests, publishes to crates.io, creates GitHub Release with changelog
- `aur-publish.yml`: Publishes AUR packages (`i3-auto-layout` and `i3-auto-layout-bin`)
...
This builds, installs, and restarts your window manager to verify the release works.
