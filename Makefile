...
PKG_BASE_NAME ?= i3-auto-layout-${CARGO_TARGET}
...
release:	## generate vendor.tar.gz and $(PKG_BASE_NAME).tar.gz
	...
	tar -czf $(PKG_BASE_NAME).tar.gz -C $(CARGO_TARGET_DIR)/$(CARGO_TARGET)/release i3-auto-layout
	@echo Released in $(CARGO_TARGET_DIR)/$(CARGO_TARGET)/release/i3-auto-layout
